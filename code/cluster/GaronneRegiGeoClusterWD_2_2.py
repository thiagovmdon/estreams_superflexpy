...
for cat_id in catchments_ids:
    ...
    unsaturated = CustomUnsaturatedReservoir(
        parameters={'Csmax': 0.3, 'Ce': 1.0, 'm': 0.01, 'beta': 2.0, 'bacon': waterdeficit_mean[cat_id]},
        states={'S0': 10.0},
        approximation=num_app,
        id='unsaturated')
...
spotpy_hyd_mod = spotpy_model(
    ...
    parameters=[
        ...
        spotpy.parameter.Uniform("snow_t0", -10.0, 10.0),
        ...
        spotpy.parameter.Uniform("unsaturated_Csmax", 0.2, 1.0),  # Global calibration parameter

    ],
    parameter_names=[
            "general_fast_k", "low_fast_k", "high_fast_k",
            "high_slow_k", "general_slow_k", "low_slow_k", "unsaturated_Ce", "snow_k", "snow_t0", "general_lowersplitter_splitpar", "high_lowersplitter_splitpar", "low_lowersplitter_splitpar",
...
sampler = spotpy.algorithms.sceua(spotpy_hyd_mod, dbname='sceua_results_regi12WDt', dbformat='csv')

sampler.sample(repetitions=35000)

#results = sampler.getdata()                                                  # Load the results
results = spotpy.analyser.load_csv_results('sceua_results_regi12WDt')
...
save_path = f"results/groups/garonne_best_params_regicomptWD_Group_2_2.csv"
