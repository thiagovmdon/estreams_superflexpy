...
def obj_fun_nsee_bfi(observations, simulation, expo=0.5, w_nsee=0.5, w_bfi=0.5, bfi_target=None):
    """
    Objective function combining NSEE and BFI mismatch.

    Parameters:
        observations (array-like): Observed discharge values.
        simulation (array-like): Simulated discharge values.
        expo (float): Exponent for NSE calculation.
        w_nsee (float): Weight for NSEE in combined score.
        w_bfi (float): Weight for BFI error in combined score.
        bfi_target (float, optional): Reference/observed BFI value. If None, it will be calculated from observations.

    Returns:
        float: Combined error score (lower is better).
    """
    observations = np.asarray(observations)
    simulation = np.asarray(simulation)
    quality_masks = np.isnan(observations).astype(int)
    ...
    # Penalty for NaNs in simulation
    if np.isnan(sim).any():
        return 10.0

    # --- NSEE ---
    nsee = np.sum((sim**expo - obs**expo)**2) / np.sum((obs**expo - np.mean(obs**expo))**2)

    # --- BFI ---
    bfi_obs = hydroanalysis.streamflow_signatures.calculate_baseflow_index(observations, quality_masks, alpha=0.925, num_filters=3, num_reflect=30, returnBF=False)
    bfi_sim = hydroanalysis.streamflow_signatures.calculate_baseflow_index(simulation, quality_masks, alpha=0.925, num_filters=3, num_reflect=30, returnBF=False)

    bfi_error = abs(bfi_sim - bfi_obs) / bfi_obs

    # --- Combined score ---
    total_error = w_nsee * nsee + w_bfi * bfi_error
...
path_inputs = 'data/models/inputvienne/subset_1988_2001'
...
combined_df = pd.read_csv("data/network_estreams_vienne_55_gauges.csv")
...
for group in group_names:
    ...
    class spotpy_model(object):
        ...
        def objectivefunction(self, simulation, evaluation):
            ...
            for sim, cat in zip(simulation, self._catchments):
                ...
                obj_value = obj_fun_nsee_bfi(observations=obs, simulation=sim, expo=0.5, w_nsee=0.5, w_bfi=0.5, bfi_target=None)
    ...
    spotpy_hyd_mod = spotpy_model(
        ...
        parameters=[
            ...
            spotpy.parameter.Uniform("general_lowersplitter_splitpar", 0.1, 0.8),
    ...
    save_path = f"results/groups/viennebfi_best_params_regi_{group}_2.csv"
