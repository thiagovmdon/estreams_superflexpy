...
found in the paper of David et al. (2022):  see <https://doi.org/10.1029/2021WR030619>.
"""
...
class UnsaturatedReservoir2(ODEsElement):
    
    """
    This class implements the UnsaturatedReservoir2 of MUFS. It consists in an adaptation
    of the proposed HBV (UnsaturatedReservoir).
    """
    ...
    def _fluxes_function_python(S, S0, ind, P, Smax, Ce, m, beta, PET, dt):
        ...
        if ind is None:
            ...
            return (
                ...
                [
                    0.0,
                    - (Ce[ind] * PET[ind] * (np.e ** (-S / m[ind]))) / m[ind],
...
class FastReservoirPower(ODEsElement):
    """
    This class implements the FastReservoirPower of MF.
    """

    def __init__(self, parameters, states, approximation, id):
        """
        This is the initializer of the class FastReservoirPower.
    ...
    @staticmethod
    def _fluxes_function_python(S, S0, ind, P, k, alpha, Ce, m, beta, PET, dt):
        # TODO: handle time variable parameters (Smax) -> overflow
    ...
    @staticmethod
    @nb.jit('Tuple((UniTuple(f8, 3), f8, f8,UniTuple(f8, 3)))(optional(f8), f8, i4, f8[:], f8[:], f8[:], f8[:], f8[:], f8[:], f8[:], f8[:])',
            nopython=True)
    def _fluxes_function_numba(S, S0, ind, P, k, alpha, Ce, m, beta, PET, dt):
        # TODO: handle time variable parameters (Smax) -> overflow
...
class FastReservoirLinear(ODEsElement):
    """
    This class implements the FastReservoirLinear of MFS. This is an adaptation 
    of the PowerReservoir present in HBV.
    """

    def __init__(self, parameters, states, approximation, id):
        """
        This is the initializer of the class FastReservoirLinear.
    ...
    @staticmethod
    def _fluxes_function_python(S, S0, ind, P, k, Ce, m, beta, PET, dt):
        # TODO: handle time variable parameters (Smax) -> overflow
    ...
    @nb.jit('Tuple((UniTuple(f8, 3), f8, f8,UniTuple(f8, 3)))(optional(f8), f8, i4, f8[:], f8[:], f8[:], f8[:], f8[:], f8[:], f8[:])',
            nopython=True)
    def _fluxes_function_numba(S, S0, ind, P, k, Ce, m, beta, PET, dt):
        # TODO: handle time variable parameters (Smax) -> overflow
