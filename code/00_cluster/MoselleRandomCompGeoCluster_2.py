...
for group in group_names:
    ...
    j = int(group.split('_')[1])

    print("Random value:", j)
    perm_areas = np.load(os.path.join(path_inputs, f'perm_areas_random{j:02d}.npy'), allow_pickle=True).item()

    # Print the first 5 entries (like .head())
    print(f"perm_areas_random{j:02d} (first 5 basins):")
    for i, (basin_id, values) in enumerate(perm_areas.items()):
        print(f"{basin_id}: {values}")
        if i == 4:
            break
    ...
    save_path = f"results/groups/moselle_best_params_randomcomp_{group}_2.csv"
